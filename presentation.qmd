...
format: 
  revealjs:
    ...
    transition: fade
    toc: false
    footer: "KSM308 Marine Ecology - Statistics"
execute: 
  echo: false
---
...
## Input data
H1: *Greater fish biomass inside than outside an MPA*
...
example_data <- tibble::tibble(
  ...
  Richness = c(16, 20, 7, 13, 12, 13, 10, 3, 1, 12, 9, 8, 10, 14, 8, 11, 2, 3)
)
...
ggplot(summary_data, aes(x = MPA, y = mean_biomass)) +
  geom_col() +
  geom_errorbar(
    aes(ymin = mean_biomass - se, ymax = mean_biomass + se),
    width = 0.2
  ) +
  theme_classic() +
...
ggplot(summary_data, aes(x = MPA, y = mean_biomass)) +
  ...
  geom_segment(
    aes(x = 1, xend = 2, y = y_star, yend = y_star),
    linewidth = 0.5,
    data = tibble()
  ) +
  geom_text(
    aes(x = 1.5, y = y_line, label = "***"),
    size = 6,
    data = tibble()
  ) +
  theme_classic() +
...
## Input data
H2: *MPA status significantly alters the ecological community*
...
elps_center <-
  ...
  mutate(
    grouping = case_when(
      Sample %in% c("S4", "S9", "S10", "S7") ~ "group1",
      TRUE ~ "group2"
    )
  ) |>
  summarise(MDS1 = mean(MDS1), MDS2 = mean(MDS2), .by = grouping)
nmds_scores |>
  mutate(
    grouping = case_when(
      Sample %in% c("S4", "S9", "S10", "S7") ~ "group1",
