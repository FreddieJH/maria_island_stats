...
pred_vars <-
  ...
  mutate(
    Kelp = sample(1:4, replace = TRUE, size = nrow(.)),
...
resp_vars_clean <-
  resp_vars %>%
  mutate_at(
    .vars = vars(-sample), # everything except 'sample' col
    .funs = replace_na,
    replace = 0
  )
...
# robust CLR (method = "robust.aitchison") is good for percentage data
distance_matrix <-
  abundance_matrix %>%
  vegdist(method = "bray")
...
elps_center <-
  nmds_vals %>%
  summarise(NMDS1 = mean(NMDS1), NMDS2 = mean(NMDS2), .by = MPA)
...
vec_env <- envfit(
  nmds,
  pred_vars_clean %>% column_to_rownames("sample"),
  na.rm = TRUE
)
vec_spp <- envfit(
  nmds,
  resp_vars_clean %>% column_to_rownames("sample"),
  na.rm = TRUE
)
